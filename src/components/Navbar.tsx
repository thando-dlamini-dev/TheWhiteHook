...
const Navbar = () => {
    ...
    return (
        <section className={`fixed m-2 rounded-lg z-50 top-0 left-0 right-0 lg:px-5 font-bold text-xl text-black p-2 lg:h-18 ${navClosed ? "h-22" : "h-screen"} flex ${!navClosed ? "flex-col bg-black/100 backdrop-blur-md" : "flex-row bg-white/15 backdrop-blur-md shadow-md"} items-center justify-between transition-all duration-300 ease-in-out`}>
            <Link to="/" className={`w-fit lg:w-fit ${!navClosed && "hidden"} lg:h-4/5 lg:flex justify-center items-center gap-1 text-2xl`}>
                <GiFishingHook className={`text-5xl bg-accent border-neutral-400 border-0 shadow-neutral-300 rounded-xl text-white p-2 mr-3`}/>
            ...
            <div className={`flex h-fit items-center justify-center gap-5 `}>
                <div
                    ...
                    {links.map((link) => <Link to={link.url} className={`${link.name === "Home" && "text-accent"} font-roxale`} key={link.name}>{link.name}</Link>)}
                </div>
            </div>
